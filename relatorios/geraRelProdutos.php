...
$sql= "SELECT a.id, b.nome as Categoria, a.nome, a.estoque_minimo, a.estoque_atual,
			 a.id_sap, a.alocacao FROM SRS_PRODUCT a LEFT JOIN SRS_CATEGORY b 
			 on b.id = a.categoria or b.nome = a.categoria ORDER BY a.nome";//, b.nome
...
$connect = mysql_connect("localhost","root","");
	...
	$exe_sql=mysql_query($sql) or die (mysql_error());
		...
		$pdf->Cell(0,1,'Almoxarifado Iveco Sete Lagoas -  RELATORIOS DE PRODUTOS',0,0,'L');
		$pdf->Ln(1);		
		...
		//$pdf->Cell(3,1,'CATEGORIA',1,0,'1');
		$pdf->Cell(7,1,'NOME',1,0,'1');
		$pdf->Cell(2,1,'EST. MIN.',1,0,'1');
	...
	while($resultado=mysql_fetch_array($exe_sql)){
		...
		//$pdf->Cell(3,1,$resultado['categoria'],1,0,'L');
		$pdf->Cell(7,1,$resultado['nome'],1,0,'L');
		$pdf->Cell(2,1,$resultado['estoque_minimo'],1,0,'L');
